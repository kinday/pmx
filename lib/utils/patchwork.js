...
/**
 * Get own property descriptors without function properties
 * @param {*} object
 * @return {Object.<string, Object>}
 */
function getStaticPropertyDescriptors(object) {
  var descriptors = getOwnPropertyDescriptors(object);
  var nativeProperties = Object.getOwnPropertyNames(Function);
  var staticPropertiesDescriptors = {};
  for (var name in descriptors) {
    if (nativeProperties.indexOf(name) === -1) {
      staticPropertiesDescriptors[name] = descriptors[name]
    }
  }
  return staticPropertiesDescriptors
}
...
function patchConstructor(Target, patches) {
  var createPatchedObject = new Function('Target', 'patches', [
    'return function Patched' + Target.name + '() {',
  ...
  var Patched = createPatchedObject(Target, patches)
  console.log(Patched)
  ...
  Object.defineProperties(Patched, getStaticPropertyDescriptors(Target));
